...
fn main() {
	...
	// INIT // (scoped so we can observe drop time)
	{
		...
		let instance = {
			...
			#[allow(unused_variables)]
			let extensions: [&'static CStr; 2] = vulkayes_window::minifb::required_extensions(&window);
		...
		// Device
		let (device, queue) = {
			let physical_devices: Vec<_> = instance.physical_devices().unwrap().collect();
			let physical_device_index = data::physical_device_index();
			let physical_device = if physical_devices.len() <= physical_device_index {
				physical_devices.into_iter().next().expect("No physical devices found")
			} else {
				physical_devices.into_iter().nth(physical_device_index).unwrap()
			};
