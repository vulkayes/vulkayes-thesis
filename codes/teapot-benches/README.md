...
The binaries need to be either run from the source of the project (e.g. the directory this readme is in) or the `VY_TEAPOT_VERT_SHADER` (default: `src/assets/vert.spv`) and `VY_TEAPOT_FRAG_SHADER` (default: `src/assets/frag.spv`) environment variables need to be set. However, for convenience, there is a copy of `src/assets` directory in the `builds` folder, so the binaries can also be easily run from there.
